...
const Package = () => {
  ...
  return (
    <div>
      <div className="bg-black min-h-screen mx-5 md:mx-20 px-5 md:px-20 py-50">
        ...
        <div className="min-w-full border-separate text-center align-middle">
          <div className="flex justify-center space-x-3 md:space-x-6 my-2 text-lg md:text-2xl lg:text-3xl relative">
            ...
            {tabs.map((tab) => (
              <button
                key={tab}
                className={`relative ${tab === activeTab ? "text-white font-bold" : "text-gray-400 hover:text-white"
                  }`}
            ...
            {tabContent[activeTab].length > 0 ? (
              tabContent[activeTab].map((trip, index) => {
                return (
                  ...
                  >
                    <h2 className="text-[24px] font-bold flex justify-center">{trip.title}</h2>
                    <div className="border border-black rounded-full border-3 flex justify-center"></div>
                    <div className="h-full">
                      <ul className={`list-disc ml-5 mt-2 px-2 grid text-justify gap-1 ${trip.activities.length >= 11
                        ? "grid-cols-3"
        ...
        {(activeTab === "Ubud" || activeTab === "Pecatu") && specialTabContent[activeTab]?.trips?.length > 0 && (
          <div className="flex justify-center my-20">
            <div className="rounded-4xl bg-white h-auto max-w-250 px-5 md:px-20 py-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10 my-10 min-w-100">
                {specialTabContent[activeTab]?.trips.map((trip, index) => (
                  ...
                  >
                    ...
                    <ul
                      className={`list-disc pl-5 grid ${trip.activities.length >= 11
                        ? "grid-cols-3"
                        : trip.activities.length >= 6
                          ? "grid-cols-2"
                          : "grid-cols-1"
              ...
              {/* Reserve Button */}
              <div className='flex justify-center mt-5'>
                <button className="hover:invert border border-white text-white flex justify-center items-center rounded-full bg-black w-60 mt-4 py-2 text-[#003000] font-semibold transition duration-300 cursor-pointer">
                  <p className="px-4">Reserve Now</p>
                  <FaArrowRight className="" />
                </button>
              </div>
